...
namespace Build
{
    class Program
    {
        ...
        private static void BuildWPFApp()
        {
            ...
            Run(paths["dotnet"],
                "publish -r win-x64 --self-contained=true /p:PublishSingleFile=true /p:IncludeNativeLibrariesForSelfExtract=true /p:Platform=\"Any CPU\" -c Release",
                @"\Launcher");
        }
        ...
        private static void CreateUpdaterZip()
        {
            ...
            TryMove(dir + @"\Core\bin\Release\netstandard2.0\Core.dll", dir + @"\autoupdate\template\VTOLVR_Data\Managed\Core.dll");
